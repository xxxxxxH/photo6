...
import kotlinx.android.synthetic.main.layout_color.*


class ColorPage:BasePage(R.layout.layout_color), OnItemClickListener {
    private var a: CommonAdapter?=null
    private var isEffect = false
    override fun start() {
        iSticker.setImageBg(selectImage)
        iTitle.setTitle(Constant.TITLE_COLOR)
        ...
        iGroup.getRadio2().apply {
            text = Constant.OPTION_FILTER
            setDrawableTop(R.mipmap.btnfilter)
        }
        ...
        iGroup.getRadioGroup().setOnCheckedChangeListener { _, checkedId ->
            when(checkedId){
                R.id.radio1 ->{
                    ...
                    getResourceBitmap(Constant.ASSETS_OVERLAY) {data->
